...
class RegisterForm(forms.ModelForm):
    ...
    password_confirm = forms.CharField(min_length=5, label='Parola Kontrol',widget=forms.PasswordInput(attrs={'class': 'form-control'}))
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email).exists():
            raise forms.ValidationError('Bu email zaten sistemde mevcut')
        return email

    def clean_username(self):
        username = self.cleaned_data.get('username')
        if User.objects.filter(username=username).exists():
            raise forms.ValidationError('Bu kullanıcı adı zaten sistemde mevcut')
        return username
...
class UserPasswordChangeForm(forms.Form):
    ...
    def clean(self):
        ...
        if new_password != new_password_confirm:
            self.add_error('new_password', 'Şifre Eşleşmedi')
            self.add_error('new_password_confirm', 'Şifre Eşleşmedi')
