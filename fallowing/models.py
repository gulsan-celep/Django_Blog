...
class Fallowing(models.Model):
    fallower = models.ForeignKey(User, related_name='fallower', null=True, verbose_name='Takip eden kullanıcı',on_delete=models.DO_NOTHING)
    fallowed = models.ForeignKey(User, null=True, related_name='fallowing', verbose_name='Takip edilen kullanıcı',on_delete=models.DO_NOTHING)
...
#kullanıcının takip edenleri getir

    @classmethod
    def get_fallowed_username(cls, user):
        fallowed = cls.get_fallowed(user)
        return fallowed.values_list('fallowed__username', flat=True)